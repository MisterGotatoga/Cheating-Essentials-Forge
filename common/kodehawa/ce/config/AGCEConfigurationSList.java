...
public class AGCEConfigurationSList {
	...
	private void create(File file, Collection list){
		DynamicLogger.instance().writeLogWithPrefix("CMLS", "Writing List config file: "+name, Level.INFO, 1);
		try
		{
	...
	private void read(){
		DynamicLogger.instance().writeLogWithPrefix("CMLS", "Reading List config file: "+name, Level.INFO, 1);
		try
		{
